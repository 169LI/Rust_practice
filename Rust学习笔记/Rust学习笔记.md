...
Rust 中的基本类型都实现了 Copy 特征，包括整数、浮点数、布尔值和字符。所有的整数类型，比如 u32。布尔类型：bool。字符类型：char。所有的浮点类型，比如 f64。如果元组包含的所有字段都是 Copy 的，那么这个元素也是 Copy 的。例如：(i32, i32) 是 Copy 的，但 (i32, String) 则不是。
