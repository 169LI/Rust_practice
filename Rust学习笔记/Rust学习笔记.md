...
## 3 运行rustup-init.exe
![](Pasted_image_20241105203307)
上图显示的是一个命令行安装向导。
如果你已经安装 MSVC （推荐），那么安装过程会非常的简单，输入 1 并回车，直接进入第二步。
![](Pasted_image_20241105203332)
...
![](Pasted_image_20241105203348)
设置完所有选项，会回到安装向导界面（第一张图），这是我们输入 1 并回车即可
## 4 测试
```
rustc -V        # 注意的大写的 V
```
![](Pasted_image_20241105203418)
## 5 Visual Studio Code 开发环境
analyzer 和 Native Debug 两个扩展   之后重启  选择一个新的文件夹
![](Pasted_image_20241105203442)
![](Pasted_image_20241105203450)
...
cargo run
```
![](Pasted_image_20241105203519)
成功的构建了一个 Rust 命令行程序！
## 6 调试（debug）
自带的那个是真不好用     自己配的这个也没好到哪里！  Rust后面会调试应该可能可以学快点（学c++不会调试直接凉一半）
![](Pasted_image_20241105203553)直接粘贴就ok了
![](Pasted_image_20241105203605)
```
{
...
![](Pasted_image_20241105203729)
为了让长数值更易读，可以在数字之间任意插入下划线。
  ![](Pasted_image_20241105203854)
